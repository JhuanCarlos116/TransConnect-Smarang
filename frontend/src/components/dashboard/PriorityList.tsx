...
/**
 * Card vocabulary from the mockup's Priority Actions panel: a photo-led
 * rank-1 card, a tagged second card, then compact rows with a red rail for
 * the critical ones.
 *
 * An earlier pass filled these with invented Location-Allocation output
 * ("+4200 served", "+3100 served" — lifted from the mockup's own made-up
 * numbers) and a "Dispatch Team" button that submitted to nothing. That
 * model is not built yet, so the cards carry what is genuinely here —
 * surveyed halte ordered by condition score — and the panel header in
 * dashboard/page.tsx says so rather than implying an optimiser ran.
 */
