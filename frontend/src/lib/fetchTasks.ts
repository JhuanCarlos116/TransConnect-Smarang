...
export async function rejectTechnicianPhoto(taskId: string): Promise<Task> {
  const res = await fetch(`${requireApiBase()}/api/v1/tasks/${taskId}/reject`, {
    method: "PATCH",
  });
  return parseOrThrow<Task>(res);
}

export async function rejectFacilityUpdate(taskId: string): Promise<Task> {
  const res = await fetch(`${requireApiBase()}/api/v1/tasks/${taskId}/reject-facility-update`, {
    method: "PATCH",
  });
  return parseOrThrow<Task>(res);
}

export async function rejectTechnicianVideo(taskId: string): Promise<Task> {
  const res = await fetch(`${requireApiBase()}/api/v1/tasks/${taskId}/reject-video`, {
    method: "PATCH",
  });
  return parseOrThrow<Task>(res);
}

export async function approveTechnicianVideo(taskId: string): Promise<Task> {
  const res = await fetch(`${requireApiBase()}/api/v1/tasks/${taskId}/approve-video`, {
    method: "PATCH",
  });
  return parseOrThrow<Task>(res);
}
